...
(popup_div) => {{
    ...
    if (sessionStorage.getItem('{load_flag}')) return true;
    ...
    proceedButton?.focus();
    proceedButton?.addEventListener('click', (e) => {{ e.stopPropagation(); e.preventDefault(); overlay.remove(); }});
    closeButton?.addEventListener('click', (e) => {{ e.stopPropagation(); e.preventDefault(); overlay.remove(); }});
