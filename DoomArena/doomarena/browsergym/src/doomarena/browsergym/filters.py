...
class StopAfterSuccessFilter(AttackFilter):
    filter_name: Literal["stop_after_success"] = "stop_after_success"
    succeeded: bool = False

    def __call__(self, observation: dict, env) -> bool:
        if self.succeeded:
            return False
        return True


class EveryNStepsFilter(AttackFilter):
    filter_name: Literal["every_n_steps"] = "every_n_steps"
    n: int = 2
    current_step: int = 0

    def __call__(self, *args, **kwargs) -> bool:
        fire = (self.current_step % self.n) == 0
        self.current_step += 1
        return fire
