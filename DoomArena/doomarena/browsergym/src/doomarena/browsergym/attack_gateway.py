...
class BrowserGymAttackGateway(AttackGateway):
    ...
    def __init__(
        ...
        self._pending_llm_response: str | None = None
        self._pending_agent_info: dict | None = None

    def set_pending_llm_response(self, response: str | None) -> None:
        """Called by the experiment loop before env.step() to pass the full
        LLM response (think + action) so success filters can inspect it."""
        self._pending_llm_response = response

    def set_pending_agent_info(self, agent_info: dict | None) -> None:
        """Called by the experiment loop before env.step() to expose agent info."""
        self._pending_agent_info = agent_info
    ...
    def _check_flags(self, page, flags) -> bool:
        ...
        for flag in flags:
            if not page.evaluate(f"window.{flag}"):
                return False
            # for popup attacks
            # if not page.evaluate(f"sessionStorage.getItem('{flag}')"):
            #     return False

        return True

    def step(self, action: str):
        pending_agent_info = self._pending_agent_info
        obs, reward, done, truncated, info = self.previous_transition
        if done:
            obs, reward, __, truncated, info = self.env.step("noop(0.1)")
            self._pending_agent_info = None
        ...
        info["task_info"]["agent_info"] = pending_agent_info
    ...
    def run_attacks_and_defenses(self, obs, reward, done, truncated, info):
        ...
        attack_contents = []
        if attack_configs:
            attack_contents = self.run_attacks(attack_configs, step_idx=self.step_counter)
            obs = self.env.unwrapped._get_obs()
        self.step_counter += 1
        ...
        for idx, attack_config in enumerate(self.attack_configs):
            ...
            if attack_config.success_filter:
                success_filter_result =  attack_config.success_filter(obs, self.env, agent_info=self._pending_agent_info)
